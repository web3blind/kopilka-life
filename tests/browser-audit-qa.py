...
def run():
    ...
    with OAuthStandIn(OAUTH_PROVIDER_URL) as oauth_provider, sync_playwright() as playwright:
        ...
        try:
            ...
            tg_story_page.add_init_script(f"localStorage.removeItem('kopilkaToken'); window.__tgStoryCall=null; window.Telegram={{WebApp:{{initData:{json.dumps(owner_init_data)},initDataUnsafe:{{}},ready(){{}},expand(){{}},version:'8.0',platform:'android',isVersionAtLeast(){{return true;}},showPopup(params,callback){{window.__tgPopup=params;window.__tgPopupCallback=callback;}},shareToStory(media,params){{if(window.__nativeConfirmed)window.__tgStoryCall={{media,params}};}}}}}};")
            ...
            require(tg_story_page.evaluate("!!window.__tgPopup && window.__tgStoryCall === null"), "Telegram must ask native confirmation before story request (TalkBack gesture)")
            tg_story_page.evaluate("window.__tgPopupCallback('cancel')")
            require(tg_story_page.evaluate("window.__tgStoryCall === null"), "Telegram cancellation must not request story")
            tg_story_page.locator("#shareStory").click()
            tg_story_page.evaluate("window.__nativeConfirmed=true; window.__tgPopupCallback('share')")
            tg_story_page.wait_for_function("window.__tgStoryCall !== null")
            require('Редактор истории открыт' not in tg_story_page.locator('#connectionStatus').inner_text(), 'Telegram API return must not claim editor opened')
